...
def reset_robot_period(env: ManagerBasedEnv, env_ids:torch.Tensor, robot_cfg: SceneEntityCfg = SceneEntityCfg("robot")):
    robot: RigidObject = env.scene[robot_cfg.name]
    try:
        robot.period
    except AttributeError as e:
        #print(e)
        robot.period = torch.zeros((env.num_envs,), device=robot.device, dtype=torch.int32)[env_ids]
    print(robot.period.max().item())
    robot.period = torch.zeros((env.num_envs, ), device=robot.device, dtype=torch.int32)[env_ids]
